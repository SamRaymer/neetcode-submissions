...
class HashTable:
    ...
    def __printLength(self):
        print("current length {length}".format(length=len(self.storage)))

    def __printKeys(self):
        storagekeys = ','.join(str(item.key) if isinstance(item, __Node__) else "None" for item in self.storage)
        print(storagekeys)
