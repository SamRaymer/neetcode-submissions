import collections
class Solution:
    toVisit: collections.deque[tuple[int, int, int]] # x, y, depth
    ...
    def shortestPath(self, grid: List[List[int]]) -> int:
        ...
        self.toVisit.append((0,0,0))
        self.visited.add((0,0))
        pathLen = None
        while pathLen == None and len(self.toVisit) > 0:
    ...
    def checkNext(self) -> int | None:
        x, y, depth = self.toVisit.popleft()
        yLen = len(self.grid)
        ...
        if x > 0 and ((x - 1, y) not in self.visited) and self.isValid(x-1, y):
            self.toVisit.append((x - 1, y, depth + 1))
        ...
        if y > 0 and ((x, y-1) not in self.visited) and self.isValid(x, y-1):
            self.toVisit.append((x, y-1, depth + 1))
