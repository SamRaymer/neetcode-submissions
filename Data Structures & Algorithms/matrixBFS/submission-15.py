...
class Solution:
    ...
    def shortestPath(self, grid: List[List[int]]) -> int:
        ...
        while pathLen == None and len(toVisit) > 0:
            ...
            if x > 0 and ((x - 1, y) not in visited) and grid[y][x-1] != 1:
                toVisit.append((x - 1, y, depth + 1))
                visited.add((x - 1, y))
            if x < (xLen - 1) and ((x + 1, y) not in visited) and grid[y][x+1] != 1:
                toVisit.append((x + 1, y, depth + 1))
                visited.add((x + 1, y))
            if y > 0 and ((x, y-1) not in visited) and grid[y-1][x] != 1:
                toVisit.append((x, y-1, depth + 1))
                visited.add((x, y - 1))
            if y < (yLen - 1) and ((x, y+1) not in visited) and grid[y+1][x] != 1:
                toVisit.append((x, y+1, depth + 1))
                visited.add((x, y + 1))
